...
public class ConcatAdapterController implements NestedAdapterWrapper.Callback {
    ...
    private WrapperAndLocalPosition findWrapperAndLocalPosition(int i) {
        ...
        throw new IllegalArgumentException(C0843a.m444e("Cannot find wrapper for ", i));
    }
    ...
    public boolean addAdapter(int i, RecyclerView.Adapter<RecyclerView.ViewHolder> adapter) {
        if (i < 0 || i > this.mWrappers.size()) {
            StringBuilder u = C0843a.m460u("Index must be between 0 and ");
            u.append(this.mWrappers.size());
            u.append(". Given:");
            u.append(i);
    ...
    public int getLocalAdapterPosition(RecyclerView.Adapter<? extends RecyclerView.ViewHolder> adapter, RecyclerView.ViewHolder viewHolder, int i) {
        ...
        StringBuilder x = C0843a.m463x("Detected inconsistent adapter updates. The local position of the view holder maps to ", countItemsBefore, " which is out of bounds for the adapter with size ", itemCount, ".Make sure to immediately call notify methods in your adapter when you change the backing dataviewHolder:");
        x.append(viewHolder);
        x.append("adapter:");
        x.append(adapter);

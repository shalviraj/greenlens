...
public class Easing {
    ...
    public static class CubicEasing extends Easing {
        ...
        public CubicEasing(String str) {
            ...
            this.f81x1 = Double.parseDouble(str.substring(indexOf + 1, indexOf2).trim());
            int i = indexOf2 + 1;
            int indexOf3 = str.indexOf(44, i);
            this.f83y1 = Double.parseDouble(str.substring(i, indexOf3).trim());
            int i2 = indexOf3 + 1;
            int indexOf4 = str.indexOf(44, i2);
            this.f82x2 = Double.parseDouble(str.substring(i2, indexOf4).trim());
            int i3 = indexOf4 + 1;
            this.f84y2 = Double.parseDouble(str.substring(i3, str.indexOf(41, i3)).trim());
        }

        private double getDiffX(double d) {
            double d2 = 1.0d - d;
            double d3 = this.f81x1;
            double d4 = d2 * 3.0d * d2 * d3;
            double d5 = this.f82x2;
            return ((1.0d - d5) * 3.0d * d * d) + ((d5 - d3) * d2 * 6.0d * d) + d4;
        }

        private double getDiffY(double d) {
            double d2 = 1.0d - d;
            double d3 = this.f83y1;
            double d4 = d2 * 3.0d * d2 * d3;
            double d5 = this.f84y2;
            return ((1.0d - d5) * 3.0d * d * d) + ((d5 - d3) * d2 * 6.0d * d) + d4;
        }
        ...
        private double getX(double d) {
            ...
            double d4 = d2 * d3 * d;
            double d5 = d3 * d * d;
            return (this.f82x2 * d5) + (this.f81x1 * d4) + (d * d * d);
        }
        ...
        private double getY(double d) {
            ...
            double d4 = d2 * d3 * d;
            double d5 = d3 * d * d;
            return (this.f84y2 * d5) + (this.f83y1 * d4) + (d * d * d);
        }
    ...
    public static Easing getInterpolator(String str2) {
        ...
        switch (c) {
            ...
            default:
                StringBuilder u = C0843a.m460u("transitionEasing syntax error syntax:transitionEasing=\"cubic(1.0,0.5,0.0,0.6)\" or ");
                u.append(Arrays.toString(NAMED_EASING));
                Log.e("ConstraintSet", u.toString());
                return sDefault;
        }
    }

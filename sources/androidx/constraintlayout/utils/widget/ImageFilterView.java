...
public class ImageFilterView extends AppCompatImageView {
    ...
    public static class ImageMatrix {
        ...
        /* renamed from: m */
        public float[] f114m = new float[20];
        ...
        public void updateMatrix(ImageView imageView) {
            ...
            if (f != 1.0f) {
                saturation(f);
                this.mColorMatrix.set(this.f114m);
                z = true;
            } else {
                z = false;
            }
            ...
            if (f3 != 1.0f) {
                warmth(f3);
                this.mTmpColorMatrix.set(this.f114m);
                this.mColorMatrix.postConcat(this.mTmpColorMatrix);
                z = true;
            }
    ...
    private void init(Context context, AttributeSet attributeSet) {
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C0190R.styleable.ImageFilterView);
            int indexCount = obtainStyledAttributes.getIndexCount();
            Drawable drawable = obtainStyledAttributes.getDrawable(C0190R.styleable.ImageFilterView_altSrc);
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0190R.styleable.ImageFilterView_crossfade) {
                    this.mCrossfade = obtainStyledAttributes.getFloat(index, 0.0f);
                } else if (index == C0190R.styleable.ImageFilterView_warmth) {
                    setWarmth(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0190R.styleable.ImageFilterView_saturation) {
                    setSaturation(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0190R.styleable.ImageFilterView_contrast) {
                    setContrast(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0190R.styleable.ImageFilterView_round) {
                    setRound(obtainStyledAttributes.getDimension(index, 0.0f));
                } else if (index == C0190R.styleable.ImageFilterView_roundPercent) {
                    setRoundPercent(obtainStyledAttributes.getFloat(index, 0.0f));
                } else if (index == C0190R.styleable.ImageFilterView_overlay) {
    ...
    public void setRound(float f) {
        ...
        if (f != 0.0f) {
            ...
            if (this.mViewOutlineProvider == null) {
                C01872 r5 = new ViewOutlineProvider() {
    ...
    public void setRoundPercent(float f) {
        ...
        if (f != 0.0f) {
            ...
            if (this.mViewOutlineProvider == null) {
                C01861 r6 = new ViewOutlineProvider() {
                    public void getOutline(View view, Outline outline) {
